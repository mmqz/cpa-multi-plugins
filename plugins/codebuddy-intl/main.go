// Package main implements the codebuddy-intl CLIProxyAPI dynamic plugin.
//
// codebuddy-intl wraps CodeBuddy Intl (www.codebuddy.ai) as a cliproxy
// provider: it performs the CodeBuddy web login flow, refreshes
// access tokens, and forwards OpenAI-compatible chat completion requests to
// the upstream /v2/chat/completions endpoint.
//
// Based on Sliverkiss/cpa-plugin/workbuddy (MIT). Adapted for codebuddy-intl:
// Global host = www.codebuddy.ai, codebuddy-intl provider name // platform=ide login entry, codebuddy-cn provider name & auth file name. auth file name.
...
const (
        providerName  = "codebuddy-intl"
        authFileName  = "codebuddy-intl.json"
        pluginLogoURL = ""
...
func wbRegistration() registration {
        return registration{
                ...
                Metadata: pluginapi.Metadata{
                        Name:             providerName,
                        Version:          version,
                        Author:           "mmqz (based on workbuddy by Sliverkiss/lovingfish)",
