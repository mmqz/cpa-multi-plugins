// Package main implements the codebuddy-cn CLIProxyAPI dynamic plugin.
//
// codebuddy-cn wraps Tencent CodeBuddy CN (copilot.tencent.com) as a cliproxy
// provider: it performs the CodeBuddy web login flow (platform=ide), refreshes
// access tokens, and forwards OpenAI-compatible chat completion requests to
// the upstream /v2/chat/completions endpoint.
//
// Based on Sliverkiss/cpa-plugin/workbuddy (MIT). Adapted for codebuddy-cn:
// platform=ide login entry, codebuddy-cn provider name & auth file name.
