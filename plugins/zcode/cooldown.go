// cooldown.go implements per-(account, model) throttling for zcode.
//
...
// auth-wide outage — especially harmful when only one zcode auth is
// configured. Throttling is therefore scoped to the failed (auth, model)
// pair, and shapes routing in two places: the per-credential model catalog is
// filtered by modelIsCooling, and the opt-in scheduler_mode=credits picker
// skips cooling candidates.
//
...
// cooldown state. Metadata values recorded by the host's execution handlers
// (the routing model before rewrite) win over req.Model.
...
// normalizeCooldownModel maps any CPA-facing spelling ("zcode/glm-5.3") onto
// the catalog key space the filter compares against.
...
func cooldownSnapshotFor(authID string) []map[string]any {
	...
	sort.Slice(out, func(i, j int) bool {
		a, _ := out[i]["model"].(string)
		b, _ := out[j]["model"].(string)
		return a < b
	})
	return out
}
...
// cooldown table. Account-level failures (hard quota exhaustion, invalid
// credentials) stay with the existing lifecycle/status handling.
