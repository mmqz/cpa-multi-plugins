...
// request with the canonical usage record. Our job is just to forward to
// CPAMP.
func handleUsage(raw []byte) ([]byte, error) {
	...
	// Only forward zcode's own records; the host routes other plugins' usage
	// to their own UsagePlugin.
...
// CPA builds) still emit usage; forwardUsageToCPAMP is idempotent at the
// CPAMP ingestion layer (NDJSON import dedups on timestamp+auth+model).
...
// usageDetailFromCompletion extracts the usage block from a non-streaming
...
// object seen (OpenAI-compatible gateways emit it on the terminal chunk).
