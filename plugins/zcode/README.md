...
- **配额面板**：`/api/v1/zcode-plan/billing/balance`（JWT + 稳定 `X-Device-Mid`，缺头会被活动网关以 3001 拒绝）实时余量，折入 creditsSummary 多池展示；账号卡显示 provider/plan/冷却，支持选中路由账号与 coding/start plan 切换。面板页免鉴权直开，但数据接口走宿主管理鉴权：密钥按 `?key=` 参数 → 面板自存副本 → 同源管理面板 localStorage 自动拾取 的顺序解析，均未命中时在面板内粘贴一次管理密钥即可（混淆存储于浏览器 localStorage）。
