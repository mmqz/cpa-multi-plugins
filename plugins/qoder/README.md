...
| **每日签到** | 面板手动签到（单账号/批量）+ 09:00/21:00 定时自动签到，签到后返回最新积分快照。v0.8.21：CN/Intl 统一走 campaigns 领取系统——上游已全局禁用 legacy CN daily-check-in（claim 恒 409 且不发积分，2026-09-21 实测）；legacy status 仅作 CN 只读统计补充 |
