...
// intlAlternateOrigins 是 Intl 平台的备用账号 API 源（cockpit-tools
// default_account_api_config：normal/sg account API + 经典 api hosts）。
var intlAlternateOrigins = []string{
...
func exchangeHosts(primary, oauth string) []string {
	...
	add(primary)
	add(oauth)
	for _, h := range intlAlternateOrigins {
		add(h)
	}
	return out
}
