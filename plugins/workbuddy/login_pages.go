...
// (v0.12.10). The CPA management UI renders plugin-declared menus as sidebar
// entries and opens them in an iframe; each login page starts a region-
// PINNED login flow (login_start), opens the upstream authorization URL in a
// new browser tab and polls login_wait until the flow completes. login_wait
// drives the SAME poll handler the host uses, then persists the credential
// via host.auth.save — so a self-serve login reuses the existing OAuth
// machinery end to end, and CN/Intl logins can run concurrently without
// touching the global login_region (which previously was the only way to
// pick a realm, forcing one OAuth entry per plugin).
//
// WorkBuddy flows are upstream-tracked: the authorization URL comes from the
// upstream auth/state endpoint and completion is detected by polling the
// upstream auth/token endpoint against the state — no plugin callback route
// and no browser origin are involved. Global (workbuddy.ai) accounts have no
// OAuth flow and keep entering through panel import.
//
// Routes (declared in managementRegistration, dispatched by handleManagement):
//
//	GET /v0/resource/plugins/workbuddy/login_cn    — login page (menu: CN 登录)
//	GET /v0/resource/plugins/workbuddy/login_intl  — login page (menu: Intl 登录)
//	GET /v0/resource/plugins/workbuddy/login_start — starts a pinned flow (JSON)
//	GET /v0/resource/plugins/workbuddy/login_wait  — polls/completes a flow (JSON)
package main
...
type loginRegionMeta struct {
	...
	region string // pinned realm: cn | intl
	title  string // page <h2> title
	hint   string // instruction shown under the spinner
}

var loginRegions = []loginRegionMeta{
	{path: "/login_cn", region: regionCN, title: "WorkBuddy CN 登录", hint: "在打开的新标签页中完成 CN（copilot.tencent.com）授权；完成后本页自动更新。"},
	{path: "/login_intl", region: regionIntl, title: "WorkBuddy Intl 登录", hint: "在打开的新标签页中完成 Intl（codebuddy.ai）授权；完成后本页自动更新。Global（workbuddy.ai）账号请在面板导入。"},
}
...
// poll budget (90 polls × 2s = 3 min) matches the 5-minute login TTL with
// headroom — the poll handler expires the flow server-side anyway.
    ...
    (function poll(){
      ...
      fetch('login_wait?state='+encodeURIComponent(p.body.state),{cache:'no-store'})
       ...
       .then(function(j){
         ...
         if(n>90){fail('等待超时，请重新发起登录。');return;}
         setTimeout(poll,2000);
       })
       .catch(function(){if(n>90){fail('等待超时，请重新发起登录。');return;}setTimeout(poll,3000);});
    })();
  })
...
// retries get "pending" instead of racing two upstream token fetches.
...
// handleLoginStart serves GET /v0/resource/plugins/workbuddy/login_start?
// region=cn|intl. It starts the region-pinned flow via the same handler the
...
// handleLoginWait serves GET /v0/resource/plugins/workbuddy/login_wait?state=….
// It reuses the host poll handler verbatim and persists the returned
// credential via host.auth.save — the same path handleImportAuth uses, so the
// account lands in the watcher/parse pipeline exactly like an imported one.
